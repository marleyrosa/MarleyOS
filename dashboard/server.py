...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .pulse-orb {{ width: 12px; height: 12px; border-radius: 50%; background: var(--green); box-shadow: 0 0 12px var(--green); animation: orb 1.8s infinite ease-in-out; }}
        ...
        .stat-card {{ background: var(--surface); backdrop-filter: blur(12px); border: 1px solid var(--border); border-radius: 12px; padding: 12px; position: relative; overflow: hidden; }}
        ...
        .stat-card::after {{ content: ""; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, var(--cyan), transparent); }}
        ...
        .status-pill.crit-pulse {{ background: rgba(255, 51, 102, 0.2); color: var(--red); border: 1px solid rgba(255, 51, 102, 0.5); }}
    </style>
</head>
<body>
    <header>
        <div class="brand">MarleyOS Cockpit</div>
    ...
    <div class="glass-panel">
        <div class="panel-header">Copilot de Engenharia Automotiva</div>
        <div id="chat-window">
            <div class="msg-bubble msg-bot"><b>Copilot:</b> Hub de telemetria ativo. Monitorando <b>ECM, BCM, TCM, BPCM, ABS e EPS</b> via MCP. Qual a sua solicitação?</div>
        </div>
        <div class="input-box">
            <input id="prompt-input" type="text" placeholder="Pergunte sobre falha de isolamento P0AA6 ou ECM..." onkeydown="if(event.key==='Enter') send()">
            <button onclick="send()">Enviar</button>
        </div>
    </div>
    <div class="glass-panel">
        <div class="panel-header">Telemetria Integrada de Bordo (Multi-ECU CAN Bus)</div>
    ...
    <script>
        async function send() {{
            ...
            try {{
                ...
                win.innerHTML += `<div class="msg-bubble msg-bot" style="color:var(--red);">Erro ao comunicar com backend MCP.</div>`;
            }}
            win.scrollTop = win.scrollHeight;
        }}
    </script>
